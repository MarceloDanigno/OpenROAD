...
vector<int> louvain_partclusmanager(vector<unsigned long long> degrees,
                                    vector<int> links,
  ...
  g.weights = weights;
  g.links = links;
  g.nb_nodes = nodes_w.size();
  g.nb_links = links.size();
  g.total_weight = g.nb_nodes;
  g.nodes_w.assign(g.nb_nodes, 1);
  g.sum_nodes_w = g.nb_nodes;
  ...
  vector<int> returnVector(g.nb_nodes);
    ...
    if (level <= returnLevel){  
      ...
      if (level == 0) {
        for (int i = 0; i < returnVector.size(); i++){
          int currentvalue = renumber[q->n2c[i]];
          returnVector[i] = currentvalue;
        }
      } else {
        for (int i = 0; i < returnVector.size(); i++){
          int currentvalue = renumber[q->n2c[returnVector[i]]];
          returnVector[i] = currentvalue;
        }
      }
      if (level == returnLevel){
  ...
  if (returnDone == false){
    cerr << "Louvain: Unable to save clustering results for the supplied level. Using step " << level << " instead."<< endl;
  }
